...
// -- Док-во корректности --
// "2 3 4 * +" → 2 + (3 * 4) = 14
// 1: Числа [2, 3, 4] → кладём в стек
// 2: Умножение → 3 * 4 = 12 → кладём 12
// 3: Сложение → 2 + 12 = 14 → кладём 14
// 4: Конец, в стеке 1 элемент: 14

// -- ВРЕМЕННАЯ СЛОЖНОСТЬ --
// Чтение входных данных (разбиение строки line.split(' ')) — O(n)
// Обход всех токенов в forEach — O(n)
// Остальное - O(1)
// Итог - O(n)

// -- ПРОСТРАНСТВЕННАЯ СЛОЖНОСТЬ --
// Дополнительная память — это память, выделенная сверх входных данных.
// Стек Stack.items создаётся отдельно и занимает O(n)
// Operations занимает O(1) и не зависит от n
// Доп. память = O(n) (стек) + O(1) (константные структуры) = O(n)

// Общая память включает всё, что хранится во время работы программы:
// Входные данные _input – O(n)
// Сам стек  – O(n)
// Operations – O(1)

// -- ПОСЫЛКА --
// https://contest.yandex.ru/contest/22t781/run-report/134282960/
...
process.stdin.on('end', () => solve(prepareData()))
...
const Operations = {
    ...
    '/': (firstOperand, secondOperand) => Math.floor(firstOperand / secondOperand),
    "+": (firstOperand, secondOperand) => firstOperand + secondOperand,
    "-": (firstOperand, secondOperand) => firstOperand - secondOperand,
}

function solve(polandNotation) {
    const stack = new Stack();

    polandNotation.forEach((notationItem) => {
        if (!(notationItem in Operations)) {
            stack.push(notationItem);
        } else {
            const secondOperand = Number(stack.pop());
            const firstOperand = Number(stack.pop());

            stack.push(Operations[notationItem](firstOperand, secondOperand));
        }
    });
